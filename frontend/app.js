...
$("#loginForm").onsubmit = async e => {
    ...
    try {
        ...
        // IMPORTANTE:
        // Oculta completamente la página principal
        // antes de mostrar el sistema.
        showApp();

        $("#sideName").textContent = d.user.name;
...
/* =========================================================
   RECUPERAR SESIÓN AL ACTUALIZAR
========================================================= */

if (token) {

    // IMPORTANTE:
    // Si ya existe sesión, la página principal
    // NO debe aparecer al actualizar.
    showApp();

    api("/me")
        .then(d => {
            $("#sideName").textContent = d.name;
        })
        .catch(() => {
            logout();
        });
    ...
    // Si no existe sesión, solamente mostramos
    // la página principal.
    showLanding();
}
...
function layout(title, sub, actions = "") {
        ...
        $("#content").innerHTML =
            ...
            <div class="card" style="margin-top:18px">
                ...
                ${
                    ...
                    `<div class="empty">
                        Agrega trabajadores y permisos para ver estadísticas.
                    </div>`
                }
...
async function loadWorkers() {
    ...
    $("#content").innerHTML =
        layout(
            ...
            `<button class="primary" onclick="workerForm()">
                + Nuevo trabajador
            </button>`
        )
...
async function filterWorkers() {
    ...
    const rows = await api(
        "/workers?search=" +
        encodeURIComponent($("#workerSearch").value)
    );
...
function workerForm(w = {}) {
    ...
    openModal(`
        ...
        <h3 class="modal-title">
            ${w.id ? "Editar trabajador" : "Nuevo trabajador"}
        </h3>
    ...
    $("#workerForm").onsubmit = async e => {
        ...
        try {
            ...
            toast("Trabajador guardado correctamente");
...
async function workerHistory(id) {
    ...
    openModal(`
        ...
        <h3 class="modal-title">
            Historial de ${esc(d.worker.names)}
        </h3>
        ...
        <div class="profile">
            ...
            <div class="card">
                ...
                <h4 style="text-align:center;margin:12px 0 0">
                    ${esc(d.worker.names)}
                </h4>
    ...
    return rows.length
        ...
        rows.map(p => `
            <tr>
                ...
                <td>${esc(p.exit_time || "-")}</td>

                <td>${esc(p.return_time || "-")}</td>
...
async function filterPermissions() {
    ...
    const u =
        "/permissions?search=" +
        encodeURIComponent($("#psearch").value) +
        "&status=" +
        encodeURIComponent($("#pstatus").value) +
        "&type=" +
        encodeURIComponent($("#ptype").value);
...
async function setPermission(id, status) {
    ...
    openModal(`
        ...
        >
            ...
            <div class="kpi-line">
                ...
                <b>${esc(p.observation || "-")}</b>
            </div>

            <div class="kpi-line">
                <span>Autorizó</span>
                <b>${esc(p.approved_by || "-")}</b>
            </div>

            <div class="kpi-line">
                <span>Comentario de decisión</span>
                <b>${esc(p.decision_reason || "-")}</b>
            </div>
    ...
    return rows.length
        ...
        rows.map(a => `
            <tr>
                ...
                <td>${esc(a.date)}</td>
                <td>${esc(a.entry_time || "-")}</td>
                <td>${esc(a.exit_time || "-")}</td>
                <td>${badge(a.status)}</td>
                <td>${a.late_minutes || 0} min</td>
                <td>${esc(a.observation || "-")}</td>
    ...
    $("#content").innerHTML =
        ...
        `<div class="grid2">
            ...
            <div class="card">
                ...
                <div class="kpi-line">
                    <span>Control</span>
                    <b>Permisos y salidas</b>
                </div>

                <div class="kpi-line">
                    <span>Asistencia</span>
                    <b>Entradas y tardanzas</b>
                </div>
